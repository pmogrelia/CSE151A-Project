...
- We chose this project because network security is one of the most pressing challenges in our increasingly digital world. Every day, malicious actors attempt to disrupt services in information or compromise systems, and these threats are often embedded within massive volumes of normal traffic. Machine learning, particularly deep learning, offers a powerful way to detect such attacks by identifying patterns that are too complex for humans or traditional rule-based systems to catch and which is what makes the project interesting and important. It demonstrates how AI can be leveraged to actively protect systems from cyberattacks in real time, essentially learning how to differentiate malicious behaviors from normal ones in ways that scale with modern internet traffic, which is what makes it a cool project. The broader impact of developing a good predictive model in this domain is huge, as it contributes to safer communication networks, helps prevent financial and data losses, and builds resilience against cyber threats, which are constantly evolving.
...
![figures1](images/figures1.png)

- The first set of figures illustrates the dataset composition. The Benign vs. Attack Distribution pie chart shows that the dataset is overwhelmingly dominated by attack traffic (92.1%), with benign traffic making up only 7.9%. This imbalance highlights why preprocessing steps like class balancing were crucial. The Botnet Type Distribution reveals that the attacks are split between Mirai (56.4%) and Gafgyt/Bashlite (43.6%). When broken down further, Attack Type Distributions for Mirai and Bashlite show the relative frequencies of different attack vectors, such as UDP floods, TCP floods, SYN floods, scans, and combination attacks. For example, Mirai traffic is dominated by UDP floods (33.5%), while Bashlite has a more even spread across UDP (33.3%) and TCP (30.3%). These figures provide a narrative foundation by showing the complexity and diversity of attacks the models must learn to distinguish. These distributions matter because intrusion detection systems must be robust not only to frequent attack types but also to rare ones. A model trained without balancing would likely overfit to UDP-based floods and fail on rare categories such as combo attacks or junk traffic. By visualizing this imbalance, the report demonstrates both the challenge and the motivation behind data balancing and careful model evaluation.

![figures2](images/figures2.png)

- The correlation heatmap of the numeric features captures the relationships between network-level statistics. Many features show strong positive correlations (deep red blocks), particularly among those measuring traffic weights and magnitudes at different layers, while others have negative or near-zero correlation. This redundancy suggests that not all features contribute independent information to the model. Such insights justify dimensionality reduction techniques like PCA, which in later steps reduced the feature set from 116 down to 7 while retaining over 95% of the variance. The heatmap not only supports the need for dimensionality reduction but also demonstrates that network traffic features often co-vary (e.g., weight, magnitude, and radius). In practice, this can help models focus on fewer but more meaningful inputs, improving efficiency without significant loss of accuracy.
...
- Data Exploration: We looked at possible options for datasets and found a decent dataset with enough observations to fit our requirements that was also interesting and usable for predictions; then downloaded the entirety of the dataset and condensed it into a single dataframe. We then printed the dataframe’s shape, the list of features, first couple rows, data types, and target/device distributions.
- Preprocessing: We checked our data for any unwanted columns/values by checking for duplicated columns and empty, null, or N/A values and removed them from our dataset, then printed out our dataset’s shape before and after cleaning. We then balanced our dataset to have equal amounts of samples for both attack and benign traffic, since we had about 6 million attack points to 500k benign. Lastly, we normalized all our columns.
- Model 1: We used a decision tree classifier model to classify our data, and used our training and testing split in addition to our original 7 million instance dataset to calculate our confusion matrix, which we printed. We tweaked our model’s hyperparameters to optimize the best confusion matrix, which was partly recommended by GPT.
- Model 2: For our unsupervised model, we decided to fit PCA on a stratified sample of our data, then use K-means clustering with k=2 to map the data into either benign or attack traffic. We then compared our predictions with the actual and printed out our accuracy, precision, recall, and f1 score.
...
![dataexp](images/dataexplorationresults.png)

### - Preprocessing:

![preprocess](images/preprocessingresults1.png)

![preprocess2](images/preprocessingresults2.png)

### - Model 1:

![model1](images/model1results.png)

### - Model 2:

![model2](images/model2results.png)
...
- The baseline DNN was useful as a starting point, but it was too simple to model the complexity of network traffic patterns, which often involve subtle and nonlinear interactions. The tuned model demonstrated why depth, regularization, and optimizer choice matter: it was able to generalize better and reduce overfitting. This reinforces the importance of balancing model complexity with techniques to ensure robustness. At the same time, the results highlighted limitations that must be addressed. Class imbalance was a clear issue, as minority attack types were more likely to be misclassified, reducing the model’s effectiveness in real-world scenarios where rare but dangerous attacks matter the most. Still, the consistency between training and validation performance suggests that the results were credible. This balance of strengths and shortcomings reflects the real scientific process, where every model provides insight, but none is perfect.
...
- For future models, we could use autoencoders with SVMs or some other supervised classification model for better results.
- A future direction we have is to export our model onto a software app that can run on a router or something that grabs internet traffic and flags attack data so that it can be applicable to the real world.
- One big thing we would've done differently is choosing the dataset. Our dataset is too easy to differentiate between benign and attack, which made our accuracies too high. Our data also had a bad ratio between attack and benign.
- Another future change we could do is optimizing how quickly our model detects attacks would add more real world applicability and would be essential to protecting users from attacks.
